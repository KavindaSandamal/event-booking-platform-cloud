...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode([
    {
      name  = "nginx"
      image = "${var.aws_account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${var.project_name}-nginx:latest"
    ...
    {
      name  = "auth"
      image = "${var.aws_account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/${var.project_name}-auth:latest"
...
# RDS Database (Free Tier Optimized)
...
resource "aws_db_instance" "main" {
  ...
  # Free tier: 7 days backup retention
  ...
  multi_az           = false  # Free tier doesn't include Multi-AZ
...
# ElastiCache Redis (Free Tier Optimized)
...
resource "aws_elasticache_replication_group" "main" {
  ...
  # Free tier: single node, no replication
...
# Amazon MQ (RabbitMQ) - Free Tier Optimized
