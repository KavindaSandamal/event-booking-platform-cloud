            ...
            const paymentPromises = bookings
                ...
                .map(async (booking) => {
                    try {
                        ...
                        const paymentResponse = await bookingAPI.get(`/booking/booking-payment/${booking.id}`);

                        if (paymentResponse.data.payment_id) {
                            // Now fetch the payment receipt using the payment ID
                            const receiptResponse = await paymentAPI.get(`/payment/payment-receipt/${paymentResponse.data.payment_id}`);
                            return {
