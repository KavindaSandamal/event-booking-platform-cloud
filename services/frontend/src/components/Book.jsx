...
export default function Book({ event, token, onClose, onBooked }) {
  ...
  const storeToken = useSelector(selectToken);
  
  // Debug logging
  console.log("Book component - prop token:", token);
  console.log("Book component - store token:", storeToken);
  console.log("Book component - localStorage token:", localStorage.getItem('token'));
  ...
  const doBook = async () => {
    ...
    try {
      const res = await bookingAPI.post("/booking/book", { event_id: event.id, seats });
      setBooking(res.data);
    } catch (e) {
