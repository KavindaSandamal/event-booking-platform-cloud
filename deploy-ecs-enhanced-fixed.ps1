# Fixed Enhanced ECS Deployment Script for Event Booking Platform
# PowerShell version for Windows

param(
    [string]$ProjectName = "event-booking-platform",
    [string]$AwsRegion = "us-west-2",
    [string]$TaskDefinitionFile = "task-definition-enhanced-clean.json"
)

Write-Host "Starting Fixed Enhanced ECS Deployment" -ForegroundColor Blue
Write-Host "================================================"
...
function Create-ECRRepositories {
    param($AwsCliPath, $AwsAccountId)
    
    Write-Host "Creating missing ECR repositories..." -ForegroundColor Yellow
    
    $repositories = @("event-booking-platform-worker")
    
    foreach ($repo in $repositories) {
        try {
            Write-Host "Creating repository: $repo" -ForegroundColor Yellow
            & $AwsCliPath ecr create-repository --repository-name $repo --region $AwsRegion | Out-Null
            Write-Host "Repository $repo created successfully" -ForegroundColor Green
        } catch {
            Write-Host "Repository $repo might already exist or creation failed" -ForegroundColor Yellow
        }
    }
}
...
function Main {
    try {
        ...
        # Create missing ECR repositories
        Create-ECRRepositories -AwsCliPath $AwsCliPath -AwsAccountId $AwsAccountId
        ...
        Write-Host "Fixed Enhanced ECS deployment completed successfully!" -ForegroundColor Green
